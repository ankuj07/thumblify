...
export const regenerateThumbnail = async (req, res) => {
    try {
        ...
        const style = req.body.style || existing.style;
        const aspect_ratio = req.body.aspect_ratio || existing.aspect_ratio;
        const color_scheme = req.body.color_scheme || existing.color_scheme;
...
async function processImageGeneration(title, user_prompt, style, aspect_ratio, color_scheme) {
    const styleKey = style;
    const colorKey = color_scheme;
    const safeAspectRatio = aspect_ratio || "16:9";
    let finalPrompt = `YouTube thumbnail for "${title}", ${stylePrompts[styleKey]}`;
    if (colorKey)
        finalPrompt += `, ${colorSchemeDescriptions[colorKey]}`;
    if (user_prompt)
    ...
    else if (safeAspectRatio === "9:16") {
        width = 768;
        height = 1344;
    }
    const stabilityResponse = await axios.post(`https://stability.ai`, {
